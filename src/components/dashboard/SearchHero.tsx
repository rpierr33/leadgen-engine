...
interface SourceOption {
  ...
  label: string;
  desc: string;
  group: "general" | "social" | "healthcare";
  industries?: string[];
}

const ALL_SOURCES: SourceOption[] = [
  // General
  { key: "web", label: "Web", desc: "Team & about pages", group: "general" },
  { key: "bbb", label: "BBB", desc: "Better Business Bureau", group: "general" },
  // Social platforms
  { key: "facebook", label: "Facebook", desc: "Business pages", group: "social" },
  { key: "instagram", label: "Instagram", desc: "Business profiles", group: "social" },
  { key: "linkedin", label: "LinkedIn", desc: "Company pages", group: "social" },
  { key: "tiktok", label: "TikTok", desc: "Business accounts", group: "social" },
  { key: "x", label: "X (Twitter)", desc: "Business accounts", group: "social" },
  // Healthcare
  { key: "healthgrades", label: "Healthgrades", desc: "Provider directory", group: "healthcare", industries: ["Healthcare"] },
  { key: "vitals", label: "Vitals", desc: "Provider ratings", group: "healthcare", industries: ["Healthcare"] },
  { key: "ahca", label: "FL Health (AHCA)", desc: "Florida licensed providers", group: "healthcare", industries: ["Healthcare"] },
];
...
export function SearchHero({ onSearch, isLoading }: SearchHeroProps) {
  ...
  const groupedSources = {
    general: availableSources.filter((s) => s.group === "general"),
    social: availableSources.filter((s) => s.group === "social"),
    healthcare: availableSources.filter((s) => s.group === "healthcare"),
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSearch(query.trim(), currentIndustry, limit, sources, minQuality > 0 ? minQuality : undefined);
  };

  const SourceCheckbox = ({ source }: { source: SourceOption }) => {
      ...
      <div className="relative z-10 px-6 py-14 sm:px-12 sm:py-18 lg:px-16 lg:py-20">
        ...
        <div className="animate-slide-up" style={{ animationDelay: "0.1s" }}>
          ...
          <p className="text-lg text-white/40 max-w-xl mb-8 leading-relaxed">
            Search by niche, city, zip code, county, or state.
        ...
        <form onSubmit={handleSubmit} className="animate-slide-up" style={{ animationDelay: "0.2s" }}>
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder='e.g. "nurse registries in 33309" or "dentists broward county"'
                value={query}
            ...
            <button
              ...
              className="group h-12 sm:h-14 px-8 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold text-sm sm:text-base transition-all duration-300 hover:from-purple-500 hover:to-blue-500 hover:shadow-[0_0_30px_oklch(0.55_0.25_270/40%)] disabled:opacity-40 flex items-center justify-center gap-2 shrink-0"
            >
              {isLoading ? (
          ...
          {showFilters && (
            <div className="mt-4 px-2 space-y-4 animate-slide-up">
              {/* Row 1: Industry, Max Companies, Min Score */}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex gap-1">
                    {MIN_QUALITY_OPTIONS.map((q) => (
                      <button key={q} type="button" onClick={() => setMinQuality(q)}
                        className={`h-10 w-12 rounded-xl text-sm font-medium transition-all ${
                          minQuality === q
                            ? "bg-purple-500/20 border border-purple-500/30 text-purple-300"
                            : "bg-white/[0.03] border border-white/[0.06] text-white/30 hover:text-white/50"
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <span className="text-[10px] text-white/15 uppercase tracking-wider w-full mb-0.5">General</span>
                  {groupedSources.general.map((s) => <SourceCheckbox key={s.key} source={s} />)}
                </div>

                {/* Social platforms */}
                <div className="flex flex-wrap gap-1.5">
                  <span className="text-[10px] text-white/15 uppercase tracking-wider w-full mb-0.5">Social Media</span>
                  {groupedSources.social.map((s) => <SourceCheckbox key={s.key} source={s} />)}
                </div>

                {/* Healthcare (conditional) */}
                {groupedSources.healthcare.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    <span className="text-[10px] text-white/15 uppercase tracking-wider w-full mb-0.5">Healthcare Directories</span>
                    {groupedSources.healthcare.map((s) => <SourceCheckbox key={s.key} source={s} />)}
                  </div>
                )}
              </div>
            </div>
